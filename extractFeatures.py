# -*- coding: utf-8 -*-
"""
Created on Fri Nov 25 09:28:16 2016

@author: Craig

extractFeatures is a package designed to deal with importing
data from various csv files, and providing them conveniently
in data structures.
"""
...
def getData():
	"""
	This function both extract values from csv files, and calculates
	min, max, skew, mean, std, of some log data.
	"""
	my_dir = os.getcwd()
	## load data from data directory.
	df_train = pd.read_csv(my_dir +'/data/train.csv')
	df_test  = pd.read_csv(my_dir +'/data/test.csv')
	df_eve   = pd.read_csv(my_dir +'/data/event_type.csv')
	df_log   = pd.read_csv(my_dir +'/data/log_feature.csv')
	df_res   = pd.read_csv(my_dir +'/data/resource_type.csv')
	df_sev   = pd.read_csv(my_dir +'/data/severity_type.csv') 
	## Process fault severity values
	# Determine number of fault_severity classes.
	y = df_train['fault_severity'].values
	num_class = max(y)+1
	#remove title from data frame
